package com.alchemists.tribetalk.pedagogy
...
/**
 * Bilingual NIPUN Bharat Worksheet Generator (Stage 4 Pedagogy Renderer).
 *
 ...
 *           saving generated PDFs directly to local device storage.
 */
class BilingualWorksheetGenerator(private val context: Context) {
    ...
    fun generateWorksheet(
        ...
        paint.textSize = 9f
        canvas.drawText("Generated offline by TribeTalk Edge AI Engine • NIPUN Bharat MTB-MLE Initiative", 45f, PAGE_HEIGHT - 35f, paint)

        pdfDocument.finishPage(page)

        // 6. Write PDF to local storage
    ...
    private fun getDefaultEntries(): List<WorksheetEntry> {
        return FLNCurriculumDatabase.items.take(10).map {
            WorksheetEntry(
                hindi = it.hindi,
                santaliOlChiki = it.santaliOlChiki,
